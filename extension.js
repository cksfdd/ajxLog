...
function getAllLogStatements(document, documentText) {
    ...
    const logRegex = /ajx.log.(print|toast|info|warn|error)\((.*)\);?/g;
    let match;
...
function deleteFoundLogStatements(workspaceEdit, docUri, logs) {
    ...
    vscode.workspace.applyEdit(workspaceEdit).then(() => {
        logs.length > 1
            ? vscode.window.showInformationMessage(`${logs.length} ajx.logs deleted`)
            : vscode.window.showInformationMessage(`${logs.length} ajx.log deleted`);
    });
}

function activate(context) {
    console.log('ajx-log-utils is now active');
    ...
    const insertLogStatement = vscode.commands.registerCommand('extension.insertLogStatement', () => {
        ...
        text
            ? vscode.commands.executeCommand('editor.action.insertLineAfter')
                .then(() => {
                    const logToInsert = `ajx.log.print('${text}---> ' + JSON.stringify(${text}));`;
                    insertText(logToInsert);
                })
            : insertText('ajx.log.print();');
